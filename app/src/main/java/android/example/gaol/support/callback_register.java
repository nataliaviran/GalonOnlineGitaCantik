...
public class callback_register extends AsyncTask<String, String, ArrayList<HashMap<String, String>>> {
    ...
    protected ArrayList<HashMap<String, String>> doInBackground(String... params) {
        ...
        paramsed.add(new BasicNameValuePair("nama", params[0])); //name
        paramsed.add(new BasicNameValuePair("username", params[1])); //email
        paramsed.add(new BasicNameValuePair("email", params[2])); //password
        paramsed.add(new BasicNameValuePair("password", params[3])); //birth date
        paramsed.add(new BasicNameValuePair("phone", params[4])); //mobile phone
        paramsed.add(new BasicNameValuePair("address", params[5]));

        System.out.println("params[0] = "+params[0]);
        System.out.println("params[1] = "+params[1]);

        JSONObject json = jsonParser.makeHttpRequest("http://dev.projectlab.co.id/mit/1417006/mst_register.php",
                "POST", paramsed);
